...
def visualize_y_prediction(model, y_vis, omega_values, normalize_func, epoch, save_dir="progress_plots"):
    """
...
# y_mid    = np.linspace   (1.0 , 3.0 , num=20)
# y_large  = np.linspace   (3.0 , 10.0, num=20)
y_train_values = np.unique(np.concatenate([y_small])).astype(np.float32)

# 归一化函数
omega_min, omega_max = omega_values.min(), omega_values.max()
y_min,       y_max       = 0.05,        3.0
...
for epoch in range(1, 10001):
    ...
    if epoch % 500 == 0:
        ...
        torch.save(model.state_dict(), f"checkpoints/siren_w0{model.net[0].act.w0:.0f}_ep{epoch}.pth")
    if epoch % 2000 == 0:
...
# === 测试 & 可视化 ===
os.makedirs("res_curves", exist_ok=True)
for y in y_test_values:   # 举例你关心的 y
    ...
    plt.tight_layout()
    plt.savefig(f"res_curves/y_{y:.3f}.png", dpi=200)
    plt.close()
    print(f"✅ Saved res_curves/y_{y:.3f}.png")
