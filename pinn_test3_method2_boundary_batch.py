...
omega_values = np.linspace(0.01, 0.1, 200)
y_values = [1, 5, 10, 40]
...
# 方法二：单独构造边界 batch（固定点监督）
bc_omega = torch.tensor([0.01, 0.02, 0.05], device=device)
bc_y = torch.tensor([1.0], device=device)
bc_input = torch.stack([bc_omega.repeat_interleave(len(bc_y)), bc_y.repeat(len(bc_omega))], dim=1)
...
for epoch in range(num_epochs):
    ...
    for i in range(0, X_train_tensor.shape[0], batch_size):
        ...
        output_bc = model(bc_input)
        loss_bc = torch.mean((output_bc[:, 0] - 1.0)**2 + (output_bc[:, 1])**2)
    ...
    if (epoch + 1) % 100 == 0:
        print(f"Epoch {epoch+1}, Loss: {epoch_loss:.6e}, Phys: {loss_phys.item():.2e}, Data: {loss_data.item():.2e}, BC: {loss_bc.item():.2e}")
...
plt.legend()
plt.grid(True)
plt.title("PINN Prediction with Boundary Batch Supervision")
plt.show()